# Several WhatsApp Business Accounts on one Dzyneco deployment
...
| --- | --- | --- |
| Phone Number ID, WABA ID, access token, verify token, two-step PIN | `whatsapp_config` (one row per Dzyneco account, token encrypted) | per account |
...
is subscribed to. Dzyneco checks that signature against
...
3. Each Dzyneco account opens Settings → WhatsApp connection and enters
...
On save, Dzyneco verifies the number with the token, registers it
...
and Dzyneco fans them out by `phone_number_id`.

Constraints:

* One phone number can be connected to one Dzyneco account only — the
...
* The **verify token** typed into the Meta App's webhook settings must
  equal the verify token saved by at least one Dzyneco account; the
...
3. Each Dzyneco account connects its number exactly as in setup A, using a
...
* Create image-header templates directly in WhatsApp Manager — once
  approved, Dzyneco lists and sends them like any other template.
