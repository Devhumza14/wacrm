...
Dzyneco ships a [Model Context Protocol](https://modelcontextprotocol.io)
...
[`dzyneco-mcp`](https://www.npmjs.com/package/dzyneco-mcp). Under the hood
...
data or send messages, add `"DZYNECO_ENABLE_WRITES": "true"` (and
`"DZYNECO_ENABLE_BROADCASTS": "true"` for mass sends) to `env`.
