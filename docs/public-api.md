...
The public API lets you drive your Dzyneco instance from your own
...
```
Authorization: Bearer dzyneco_live_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
```
...
3. Copy the key. **The full key is shown exactly once.** Dzyneco
...
curl https://your-crm.example.com/api/v1/me \
  -H "Authorization: Bearer dzyneco_live_xxx"
```
...
Rather than polling, register an endpoint and Dzyneco will POST to it when
...
- `POST /api/v1/webhooks` — register `{ "url": "https://…", "events": ["message.received"] }`. `url` must be `https://`. **The response includes `secret` exactly once** — store it to verify signatures; Dzyneco keeps only an encrypted copy.
...
curl -X POST https://your-crm.example.com/api/v1/webhooks \
  -H "Authorization: Bearer dzyneco_live_xxx" \
  -H "Content-Type: application/json" \
  -d '{ "url": "https://example.com/hooks/Dzyneco", "events": ["message.received"] }'
...
Headers: `X-Dzyneco-Event`, `X-Dzyneco-Webhook-Id`, and `X-Dzyneco-Signature`.

### Verifying the signature

`X-Dzyneco-Signature: t=<unix_seconds>,v1=<hex>` where `v1 =
...
covers messages Dzyneco stores (inbox + API sends), not broadcast-only
...
[#245](https://github.com/ArnasDon/Dzyneco/issues/245). Future ideas
