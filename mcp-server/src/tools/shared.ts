...
// layer dumb). On a DzynecoApiError we return an `isError` result with
...
import { DzynecoApiError } from '../client.js';
...
/**
 * Wrap a tool handler so any DzynecoApiError becomes a clean, model-
...
export function handle<A>(
  ...
  return async (args: A) => {
    try {
      ...
      if (err instanceof DzynecoApiError) {
        return errorResult(`Dzyneco API error [${err.code}]: ${err.message}`);
      }
