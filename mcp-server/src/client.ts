// ============================================================
// Dzyneco public API client.
//
// A thin wrapper over the `/api/v1` REST surface. It attaches the
// bearer key, unwraps the `{ data }` / `{ error }` envelope, and
// turns API failures into a typed DzynecoApiError the tools can render
// cleanly. Nothing here knows about MCP — it's just the CRM API.
// ============================================================

import type { Config } from './config.js';

/** A structured error from the Dzyneco API envelope (`{ error: { code, message } }`). */
export class DzynecoApiError extends Error {
  readonly status: number;
  readonly code: string;

  constructor(status: number, code: string, message: string) {
    super(message);
    this.name = 'DzynecoApiError';
    this.status = status;
    this.code = code;
  }
}
...
export class DzynecoClient {
  ...
  private async request<T>(
    ...
    try {
      ...
      throw new DzynecoApiError(
        0,
        'network_error',
        `Could not reach Dzyneco at ${this.baseUrl}: ${(err as Error).message}`,
      );
    }
    ...
    if (text) {
      try {
        ...
        if (!res.ok) {
          throw new DzynecoApiError(res.status, 'internal', text.slice(0, 500));
        }
      }
    }
    ...
    if (!res.ok) {
      ...
      throw new DzynecoApiError(res.status, code, message);
    }
