...
export function explainMetaError(
  ...
  if (code === 133010) {
    return build(
      'This phone number is not registered with the WhatsApp Cloud API yet. Enter the two-step ' +
        'verification PIN below and save again so Dzyneco can register it (POST /register).',
      'pin',
      'user',
    )
  }
  ...
  if (code === 131031) {
    return build(
      'Meta has restricted or locked this WhatsApp Business Account, so nothing in Dzyneco can ' +
