...
//
// Every delivery carries an `X-Dzyneco-Signature` header so receivers
// can verify the request really came from Dzyneco and wasn't tampered
// with or replayed. The scheme is Stripe-style:
//
//   X-Dzyneco-Signature: t=<unix_seconds>,v1=<hex HMAC-SHA256>
//
...
/**
 * Build the `X-Dzyneco-Signature` header value for `rawBody`, signed
...
/**
 * Verify a signature header. Exposed so a Dzyneco-to-Dzyneco integration
