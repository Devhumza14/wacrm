...
describe('hashApiKey', () => {
  it('is deterministic', () => {
    expect(hashApiKey('dzyneco_live_abc')).toBe(hashApiKey('dzyneco_live_abc'));
  });

  it('differs for different inputs', () => {
    expect(hashApiKey('dzyneco_live_abc')).not.toBe(hashApiKey('dzyneco_live_abd'));
  });
});
...
describe('timingSafeHexEqual', () => {
  it('is true for identical digests', () => {
    const h = hashApiKey('dzyneco_live_xyz');
